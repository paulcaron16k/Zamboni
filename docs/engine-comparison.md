...
| 9 | **Spark can skip the directory listing** — `file_list_view` ("dataset to look for files in, skipping the directory listing") and `prefix_listing`. Spark is alone in this: Trino lists, and so does Zamboni | An *alternative* answer to the remote-signing warehouse problem: take the file list from an inventory report instead of listing. Zamboni's answer is different — it lists on the object store's own credentials (design.md §6.4a), which needs no external input. **Considered and declined** (ZMBNI-94): the invariants were restated for an externally supplied list in [design.md §6.6a](design.md), and the blocker is economics rather than safety — measured on one table, the listing is ~50ms against ~2035ms to compute the reachable set, so an inventory optimises 2% of the operation. See [ice-keeper-comparison.md](ice-keeper-comparison.md) |
| 10 | **`use-starting-sequence-number`** (default true) — Spark stamps the rewrite with the sequence number at compaction start | Zamboni preserves sequence numbers exactly in manifest rewriting; the semantics need confirming against each other before either is described as equivalent || 10 | **How each engine obtains its candidate list**, from the connector sources rather than the docs. Trino: `fileSystem.listFiles(Location.of(table.location()))` — one recursive listing of the table's own location — then `entry.lastModified().isBefore(expiration) && !validFiles.contains(entry.location().fileName())`. Spark: a listing unless `file_list_view` supplies one. Zamboni: `_list_one` per root from `storage_roots(tbl)`, recursive | All three scope to the table, none sweeps the warehouse. The difference that matters is the **comparison key**: Trino matches on `fileName()` alone, and its own source concedes the risk — "in practice Iceberg metadata and data files have UUIDs in their names which makes this unlikely". Zamboni compares canonical `bucket/key` paths (`reachable.canonical`), so two files sharing a basename in different directories cannot be confused. Trino also applies the age guard *during* the listing, where Zamboni applies it after the diff |
