...
| 9 | **Spark can skip the directory listing** — `file_list_view` ("dataset to look for files in, skipping the directory listing") and `prefix_listing` | An *alternative* answer to the remote-signing warehouse problem: take the file list from an inventory report instead of listing. Zamboni's answer is different — it lists on the object store's own credentials (design.md §6.4a), which needs no external input and so keeps the ordering invariant of §6.6 intact. An inventory-report path remains unbuilt (ZMBNI-1602). See [ice-keeper-comparison.md](ice-keeper-comparison.md) |
