...
**Story counts:** 84 done · 2 inproject · 10 todo · 1 cancelled  (97 stories)
...
| ZMBNI-1101 | The branch and the path dependency | `feature/pyiceberg-0.12` against the `../iceberg-python` checkout, with the lock-file consequence documented where someone will hit it **Done.** Branch `feature/pyiceberg-0.12` builds against `../iceberg-python` @ `154288fb` via `[tool.uv.sources]`. Two setup obstacles worth recording: the checkout declares version `0.11.0` while being *newer* than 0.11.1 (main is not bumped for the cycle), so the `>=0.11.1` constraint rejected it; and building it compiles a Cython Avro decoder, which needs Python headers the system interpreter lacked — resolved with a uv-managed CPython rather than sudo. Suite on 0.12: **393 passed, 9 failed, 5 errors** | done | 2026-08-05 |
| ZMBNI-1102 | Re-probe and record the delta | Three of seven probes flip: streaming writes, manifest predicate pruning, and derives-delete-predicate, all False → True. The last two are the pair whose *split* would make Zamboni refuse to run; they land together, which is why this migration is lower risk than its size suggests. Assert the `doctor` output rather than describing it **Done, and it caught a real defect.** Re-probing against 0.12 showed `equality_deletes_readable` **True** while PyIceberg was still refusing equality deletes one call deeper — the probe inspected `DataScan._plan_files_local`, which 0.12 reduced to a five-line delegation. Fixed to search the module. The delta is now *asserted*: four tests check each probe against the installed source rather than describing it, and the equality one is verified to fail if the probe is narrowed back. Three probes flip on 0.12 — streaming writes, manifest pruning, delete-predicate derivation — the last two together, which is what keeps the build usable | done | 2026-08-05 |
| ZMBNI-1103 | Audit the private APIs we drive | The real work: 397 commits since 0.11.1. Known movers — `_scan_plan_helper` renamed to `_plan_manifest_entries`, `BaseScan`/`ManifestGroupPlanner` extracted, and `ManifestEntry.snapshot_id`'s setter fixed for writing to the wrong index, which `manifests.py` depends on preserving exactly **Inventory taken by ZMBNI-1101, two clusters.** (1) **Partition evolution duplicates rows** — 3 tests, `[3,3,4,4]` becomes `[3,3,3,3,4,4,4,4]`. Single-spec compaction is unaffected (17/17 pass), so it is the multi-spec path. That is precisely the double-counting `manifest_pruning_is_safe` exists to prevent, and the probe reports **safe** — both `prunes_manifests_by_predicate` and `derives_delete_predicate` are True on 0.12, so `unsupported_reason()` returns None and we would allow the run. Root cause unconfirmed; the hypothesis is that a predicate derived from removed files does not match manifests written under a *different* spec, so those are kept verbatim. (2) **`upsert` gained a duplicate-row guard** — `ValueError: Target table has duplicate rows`, breaking 11 demo tests. Needs deciding whether the demo data is genuinely duplicated or the guard is over-strict **Both clusters resolved.** (1) Evolution row duplication **fixed**: `MultiSpecReplaceFiles` overrides `_manifests()` and so inherited 0.12's pruning `_existing_manifests()`, which appends a non-matching manifest *verbatim*. Calling the predicate builder is not enough — the predicate compares a source column to a partition ordinal. Replaced with `_surviving_manifests()`, matching entries directly. Verified on both 0.11.1 and 0.12. (2) The demo `upsert` failures are an **upstream data-corruption regression**, not ours — reproduced in 25 lines with no Zamboni code: on a partitioned table `upsert` leaves the superseded row *and* duplicates an untouched one. Same mechanism. [upstream-0.12-upsert-regression.md](upstream-0.12-upsert-regression.md) | done | 2026-08-05 |
| ZMBNI-1104 | Adopt streaming writes | `_dataframe_to_data_files` now accepts `pa.RecordBatchReader`, so the writer bin-packs the stream itself. May retire part of the chunked backend; measure before deleting **Measured; nothing deleted.** The delegation already existed and gates on the probe, so it activates on 0.12 by itself. `_bin_pack` **cannot** be retired: upstream streaming is unpartitioned-only (apache/iceberg-python#2152), and partitioned is our common case. Replacing it with 0.12's `bin_pack_record_batches` would add a branch rather than remove code while 0.11.1 is supported — revisit when the floor moves. The real gap was coverage: both chunked tests use a partitioned table, so the streaming branch had **never executed** on any build. Now pinned in both directions, skipping honestly where the build cannot support it | done | 2026-08-05 |
| ZMBNI-1105 | Re-verify on 0.12 | Full suite plus live verification. Neither waited-on blocker lifts — `ManifestWriterV2.content()` still returns `DATA`, there is still no `ManifestWriterV3`, and the equality-delete guard is still present — so ZMBNI-604 and 704–706 stay blocked and should be re-confirmed, not assumed **Done.** 418 pass on 0.12 with the dev stack live, and **every** failure is the demo's ingest — zero non-demo failures. All maintenance operations work against a real Lakekeeper and MinIO on 0.12; only `upsert` is broken, and that is upstream. Both blockers re-confirmed rather than assumed: `ManifestWriterV2.content()` still returns `DATA`, there is still no `ManifestWriterV3`, and the equality-delete guard is still present, so ZMBNI-604 and 704–706 stay blocked. [upstream-0.12-upsert-regression.md](upstream-0.12-upsert-regression.md) | done | 2026-08-05 |
| ZMBNI-1106 | Decide the support window | One PyIceberg line or two. Settled as an architecture question by ZMBNI-1107: with probe-driven capabilities, supporting both needs no version branching in code at all. What remains is CI spend — a matrix where some expected values differ by install. Open question 1 in roadmap.md **Evidence gathered; recommendation made; decision left open deliberately.** Architecturally settled by ZMBNI-1107 — probe-driven capabilities mean **no version branching in the source**. Measured cost of two lines: one mypy flag (`warn_unused_ignores`), two tests that skip by build, and a second CI leg. Against that, 0.12 lifts **none** of our blockers and currently breaks `upsert` on partitioned tables. Recommendation: stay on 0.11.1, keep the branch, move the floor once 0.12 ships *and* the regression is fixed. A release-timing call, so it stays open for someone to make deliberately | inproject | |
