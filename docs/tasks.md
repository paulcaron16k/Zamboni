...
**Story counts:** 94 done · 1 inproject · 17 todo · 1 cancelled  (113 stories)
...
| ZMBNI-1906 | Bound compaction memory | The ceiling ZMBNI-1901 measured: peak RSS is ~2x a rewrite group, and a group is one (spec, partition) with no cap — so an unpartitioned table is compacted whole. Two candidate levers, neither free. **Capping group size** bounds memory immediately and silently degrades every Z-ordered table: clustering quality is a function of how many rows the sort can see at once, so N sub-groups give N overlapping ranges instead of one ordered sequence and file skipping falls off roughly in proportion. **Reducing the per-group ratio** is the better half of the problem — consuming PyIceberg's `ArrowScan.to_record_batches` and discarding every batch still grows ~1.3x, so most of the ceiling is upstream buffering rather than our bin-packing. If a cap ships it must be explicit and must refuse to apply itself silently to a table that asked for Z-order. Partitioning is the lever that has no such conflict, since Z-order applies within a partition anyway — documented in [user_guide.md](user_guide.md) as the answer until this lands | todo | |
