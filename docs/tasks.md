...
**Story counts:** 96 done · 1 inproject · 17 todo · 1 cancelled  (115 stories)
...
| ZMBNI-1909 | Bounded read-ahead | ZMBNI-1908 shows the whole cost is serialised round trips, so two to four files in flight would recover most of it while keeping peak proportional to the read-ahead rather than to the group — the bound stays, the parallelism comes back. Needs a config knob and a decision on the default; 1 is today's behaviour and is the safe floor. Worth doing before anyone points this at a cross-region bucket | todo | |
