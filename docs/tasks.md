...
**Story counts:** 88 done · 1 inproject · 17 todo · 1 cancelled  (107 stories)
...
| ZMBNI-913 | Spark in the dev stack and in CI | A standalone Spark service behind a `spark` compose profile, mirroring what ZMBNI-14 did for Trino, plus live tests and a CI job. Today the Spark maintainer's automated coverage is the SQL strings it generates: `tests/test_maintainers.py` builds statements with no JVM, and nothing sends one to a real session. Everything ZMBNI-1506 established — that Spark 3.5.9 accepts these procedure calls, the 24-hour orphan floor, the Hadoop-FileSystem credential split, 6 data files to 1 — was a manual local run and is not repeatable. That gap is why the timezone defect reached review: no test ever ran a statement against a session whose `spark.sql.session.timeZone` was not UTC. **The session must therefore be configured to a non-UTC zone deliberately**, since a UTC-defaulted one would pass the buggy code. Three things to decide rather than assume: whether the service exposes Spark Connect (`sc://`, no JVM on the developer's machine, but `SparkMaintainer.connect()` has no `remote` option today and would need one) or a standalone master (`spark://`, which still starts a local driver JVM and so still pins the developer's Java version); that `spark.hadoop.fs.s3a.*` must be configured separately from the catalog's vended credentials, or `remove-orphans` alone fails while the other five pass; and whether CI runs this per-PR or on a schedule — the image and JVM start make it the heaviest job in the file, and the Trino job is already the long pole | todo | |
